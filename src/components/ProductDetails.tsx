...
import { Star, X, ExternalLink, Cpu, Cpu as Gpu, MemoryStick as Memory, HardDrive, ChevronLeft, ChevronRight } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <div className="relative">
                <img
                  src={build.images && build.images.length > 0 ? build.images[selectedImage] : build.image}
                  alt={`${build.name} - view ${selectedImage + 1}`}
              ...
              <div className="mt-4 flex gap-2 overflow-x-auto pb-2">
                {build.images && build.images.map((img, index) => (
                  <button
                    key={index}
                    onClick={() => setSelectedImage(index)}
                    className={`flex-shrink-0 w-16 h-16 border-2 rounded overflow-hidden ${selectedImage === index ? 'border-blue-500' : 'border-gray-200'}`}
                  >
                    <img
                      src={img}
                      alt={`${build.name} thumbnail ${index + 1}`}
